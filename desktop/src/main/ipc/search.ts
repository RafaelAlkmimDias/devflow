...
export function registerSearchHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (useRipgrep) {
        return searchWithRipgrep(rootPath, escapedQuery, {
          fileTypes,
          caseSensitive,
          maxResults,
        })
      } else {
        return searchWithNodejs(rootPath, query, {
          fileTypes,
          caseSensitive,
          maxResults,
        })
  ...
  ipcMain.handle('search:files', async (_, rootPath: string, query: string): Promise<string[]> => {
